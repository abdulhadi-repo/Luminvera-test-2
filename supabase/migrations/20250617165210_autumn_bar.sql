/*
  ...
  1. Sample Categories
    - Insert categories with proper conflict handling
    - Use UPSERT pattern to avoid duplicate key errors

  2. Sample Products
    - Insert products with proper conflict handling
    - Link to existing categories

  3. Update product counts
    - Trigger will automatically update category product counts
*/

-- Insert sample categories using UPSERT to handle existing data
...
-- Insert sample products using UPSERT to handle existing data
